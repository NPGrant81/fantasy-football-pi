...
describe('UAT mock fixtures sanity', () => {
  ...
  test('matchup JSON totals match player scores', () => {
    const calcTotal = (team) =>
      team.players.reduce((sum, p) => sum + p.score, 0);
    expect(calcTotal(matchupData.home_team)).toBeCloseTo(
      matchupData.home_team.total_score
    );
    expect(calcTotal(matchupData.away_team)).toBeCloseTo(
      matchupData.away_team.total_score
    );
  });
