...
export default function DropPlayerModal({ isOpen, onClose, myRoster, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl">
        ...
        <h2 className="text-2xl font-black uppercase italic text-white mb-2">Roster Full!</h2>
        <p className="text-slate-400 text-sm mb-6">Select a player to release to make room for your new addition.</p>
          ...
          {myRoster.map(player => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span className={`text-[9px] font-black px-2 py-1 rounded ${getPosColor(player.position)}`}>
                  {player.position}
                </span>
                <span className="text-white font-bold group-hover:text-red-400">{player.name}</span>
              </div>
              <span className="text-[10px] font-black text-slate-600 uppercase italic group-hover:text-red-500">Drop</span>
            </button>
          ))}
        </div>
