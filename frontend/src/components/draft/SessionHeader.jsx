...
export default function SessionHeader({ sessionId, rosterSize, onFinalize }) {
  ...
  const handleFinalize = () => {
    if (window.confirm("WARNING: This will finalize all picks and close the auction. Proceed?")) {
      onFinalize();
    }
  };
    ...
    <div className="flex justify-between items-center py-2 bg-slate-950 border border-slate-800 rounded-lg px-4 mb-6 shadow-inner uppercase tracking-[0.2em] text-[9px] font-black">
      ...
      <div className="flex gap-8 items-center">
        <div className="flex items-center gap-2 group">
          ...
          <span className="text-slate-500">Session ID: <span className="text-purple-400 font-mono ml-1">{sessionId}</span></span>
        </div>
        
        <div className="flex items-center gap-2 group">
          <FiUsers className="text-blue-500 group-hover:scale-110 transition-transform" />
          <span className="text-slate-500">Roster Limit: <span className="text-blue-400 font-mono ml-1">{rosterSize}</span></span>
        </div>
      </div>
