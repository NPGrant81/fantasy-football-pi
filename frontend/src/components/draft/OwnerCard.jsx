...
const OwnerCard = memo(({ 
  ...
  return (
    <div className={`flex flex-col h-[400px] rounded-2xl border relative transition-all duration-300 ${cardStatusClass}`}>
      ...
      {/* 2.3 HEADER SECTION */}
      <div className={`p-4 border-b flex justify-between items-start bg-black/20 rounded-t-2xl ${
        ...
        <div className="w-2/3">
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {POSITIONS.map(pos => {
              ...
              return (
                <span key={pos} className={`text-[9px] font-bold px-1.5 py-0.5 rounded border transition-colors ${
                  count > 0 
        ...
        <div className="text-right w-1/3">
          <div className={`text-2xl font-mono font-black leading-none ${
            isBudgetLow ? "text-red-500 animate-pulse" : "text-green-400"
          }`}>
            ${stats.budget}
          </div>
          <div className="text-[10px] text-slate-500 uppercase font-bold mt-1">Max: ${stats.maxBid}</div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar bg-slate-950/20">
        {myPicks.map(p => {
          ...
          return (
            ...
            >
              ...
              <span className="font-mono font-black text-xs">${p.amount}</span>
            </div>
          );
        })}
        ...
        {[...Array(stats.emptySpots)].map((_, i) => (
          ...
          >
            <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">Open Slot</span>
          </div>
        ))}
      </div>
    </div>
