...
export default function Dashboard({ ownerId }) {
  ...
  useEffect(() => {
    ...
    apiClient.get(`/dashboard/${ownerId}`)
      .then(res => setSummary(res.data))
      .catch(err => console.error("Dashboard fetch failed", err));
  }, [ownerId]);

  if (!summary) return <div className="p-10 text-center animate-pulse text-slate-500 font-black uppercase">Loading your locker room...</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 text-white min-h-screen">
      ...
      <div className="flex justify-between items-end mb-12 border-b border-slate-800 pb-8">
        <div>
          <h1 className="text-6xl font-black italic uppercase tracking-tighter leading-none">Your Locker Room</h1>
          <p className="text-slate-400 mt-4 flex items-center gap-2">
            Current Standing: <span className="bg-purple-600 text-white px-3 py-1 rounded-lg font-black italic">#{summary.standing} Place</span>
          </p>
        </div>
        ...
        <div className="flex gap-4">
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl text-center min-w-[140px] shadow-2xl">
            ...
            <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Pending Trades</div>
        ...
        <div className="lg:col-span-8 bg-slate-900/40 border border-slate-800 rounded-[2.5rem] p-8 backdrop-blur-sm">
          ...
          <div className="grid grid-cols-1 gap-3">
            {summary.roster.map(player => (
              <div key={player.id} className="flex justify-between items-center p-5 bg-slate-950/50 border border-slate-800/50 rounded-2xl hover:border-blue-500/50 hover:bg-slate-900/50 transition-all duration-300 group">
                <div className="flex items-center gap-5">
                  <span className={`text-[10px] font-black px-3 py-1.5 rounded-md shadow-lg ${getPosColor(player.position)}`}>
                    {normalizePos(player.position)}
                  </span>
                  <span className="font-bold text-xl tracking-tight group-hover:text-blue-400 transition-colors">{player.name}</span>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-slate-500 font-mono text-sm font-bold uppercase tracking-widest bg-slate-900 px-3 py-1 rounded-lg border border-slate-800">{player.nfl_team}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-sm text-slate-400 mb-8 leading-relaxed">
              The wire is hot. Browse available free agents to fortify your lineup before the next window.
            </p>
            <Link 
              to="/waivers" 
          ...
          <div className="bg-slate-900/80 border border-slate-800 p-8 rounded-[2.5rem]">
            ...
            <ul className="space-y-6">
              <li className="relative pl-6">
                ...
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Waiver Deadline</p>
              ...
              <li className="relative pl-6 opacity-60">
                ...
                <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Draft Status</p>
                <p className="text-white font-black text-lg tracking-tight uppercase">Draft Finalized</p>
              </li>
            </ul>
          </div>
