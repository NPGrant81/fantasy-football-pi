...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2002 | `29721` | `www47` | `pass` | `pass` | `partial` | `partial` | Draft parser still yields `0` usable API draft rows (order-only picks), but stage now scaffolds `160` manual skeleton rows in `backend/exports/history_staged_2002/manual_overrides/draftResults/2002.csv` (franchise/round/pick prefilled). Controlled probe root `backend/exports/history_probe_275` validated dry-run + apply + reconcile (`draft_picks_inserted=1`, `mismatch_count=0`), confirming the apply lane works when owner/player mappings exist. |
