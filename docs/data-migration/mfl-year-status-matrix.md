...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2002 | `29721` | `www47` | `pass` | `pass` | `partial` | `partial` | Draft parser now emits only picks with player IDs, yielding `0` usable draft rows for current API/XML payloads (order-only picks). Staged roots under `backend/exports/history_staged_2002` remain importer-compatible and now auto-create `manual_overrides/draftResults/2002.csv` for operator backfill. |
| 2003 | `39069` | `www44` | `pass` | `pass` | `partial` | `partial` | Draft parser now emits only picks with player IDs, yielding `0` usable draft rows for current API/XML payloads (metadata-only draft payload). Staged roots under `backend/exports/history_staged_2003` remain importer-compatible and now auto-create `manual_overrides/draftResults/2003.csv` for operator backfill. |
