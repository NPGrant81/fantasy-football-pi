...
- Threads opened: 6
- Threads resolved: 6
- Follow-up commits: `b7fd9c5`, `8453125`
...
- Validation: all 17 workflows parsed; `actionlint` and focused classifier/gate scenarios passed; final-head GitHub Actions passed every applicable workflow and `ci-gate`.
- Residual risk: Copilot re-review must be requested in the GitHub UI because GitHub's REST and GraphQL reviewer APIs reject bot identities.
...
- Threads opened: 9
- Threads resolved: 9
- Suppressed findings reviewed: 9
...
- Maintenance hardening: ESPN/NFL importers support package/direct execution without import-time sessions, schedule generation owns and closes its session, and partial-index downgrade drops by name only.
- Latest broad backend validation: `783 passed, 1 skipped`.
...
- Partial-index proof: explicit upgrade predicate and predicate-free drop passed PostgreSQL bootstrap introspection and full downgrade.
